...
p.addParameter('fileColID', [2, 4, 15, 16, 24, 25, 26, 27, 32, 51, 95, 96, 98, 9, 10, 11, 12, 13, 14, 102, 103]);
...
p.addParameter('SetSQL', ...
            {'Timestamp = ADDTIME(STR_TO_DATE(@Date_UTC, ''%Y-%m-%d''), STR_TO_DATE(@Time_UTC, '' %H:%i''))',...
            ...
            'Displacement = nullif(@Displacement * 1.025, '''')',...
            'Latitude = @Latitude_Deg + @Latitude_Min/60 * CASE @Latitude_NS WHEN ''North'' THEN 1 WHEN ''South'' THEN -1 ELSE NULL END',...
            'Longitude = @Longitude_Deg + @Longitude_Min/60 * CASE @Longitude_EW WHEN ''East'' THEN 1 WHEN ''West'' THEN -1 ELSE NULL END'});
