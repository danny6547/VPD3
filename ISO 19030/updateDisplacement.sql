...
proc_something:BEGIN
    ...
    SET displacementGiven = (SELECT COUNT(*) FROM `inservice`.tempRawISO WHERE Displacement IS NOT NULL) > 0;
    
    IF displacementGiven THEN
		LEAVE proc_something;
	END IF;
        
	IF (SELECT Block_Coefficient FROM `static`.Vessels WHERE IMO_Vessel_Number = IMO) IS NULL THEN
	
		/* Calculate Displacement based on Hydrostatic Table */
		UPDATE `inservice`.tempRawISO SET Displacement = (SELECT Displacement FROM `static`.Displacement WHERE IMO_Vessel_Number = imo AND
    ...
    ELSE
        ...
        SET AdjustedTopArea := (SELECT Block_Coefficient*Length_Overall*Breadth_Moulded FROM `static`.Vessels WHERE IMO_Vessel_Number = IMO);
        UPDATE `inservice`.tempRawISO SET Displacement = AdjustedTopArea*( (Static_Draught_Fore + Static_Draught_Aft) / 2);
