...
BEGIN
    ...
    SET @Za = ( SELECT Anemometer_Height FROM `static`.Vessels WHERE IMO_Vessel_Number = @imo );
    ...
    */
    UPDATE `inservice`.tempRawISO SET Relative_Wind_Speed_Reference = SQRT(POWER(WVX + Speed_Over_Ground, 2) + POWER(WVY, 2));
