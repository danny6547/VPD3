...
methods(Static)
    ...
    wind.Coefficient = zeros(size(wind.Direction)); %[1.2640,1.3450,1.4090,1.3890,1.3150,1.1430,0.9230,0.6840,0.5100,0.3200,0.0780,-0.2070,-0.5560,-0.9420,-1.2350,-1.4660,-1.5740,-1.4630,-1.2570];
...
%     blockSize(blockSize == 0) = [];
    ...
    function obj = runISOOnInput(obj)
    % runISOOnInput
        ...
        rawFile = obj.RawFile;
        q = fileparts(rawFile);
        rawFile = fullfile(q, 'validateISO19030Test_mps.csv');
        tab = 'RawData';
        cols_c = {'DateTime_UTC',...
                    'Speed_Through_Water',...
                    'Delivered_Power',... Delivered Power
    ...
    function obj = compareOutputsDitteDamcl(obj)
    % compareOutputsDitte
        
        % Get DIGU values
        ISO_test;
        digu_pv = data.performancedata.PV;
        
        % Get DAMCL values
        obj = obj.readDBISOTable;
        dmcl_pv = obj.DBISOTable.speed_loss;
        
        % Get common points
        digu_time = data.performancedata.date;
        dmcl_time = obj.DBISOTable.datetime_utc;
        [~, q, w] = intersect(digu_time, dmcl_time);
        digu_pv = digu_pv(q);
        dmcl_pv = dmcl_pv(w);
        
        % Find similarity
        p = polyfit(digu_pv*100, dmcl_pv, 1);
        slope = p(1);
        
        % Plot comparison
        figure;
        plot(digu_pv*100, dmcl_pv, 'b*');
        annotation('textbox',[.2 .5 .3 .3],...
                    'String',['Slope = ', num2str(slope)],...
                    'FitBoxToText','on');
    end
    
    function obj = compareOutputsDiguSoftware(obj)
    %     
        ...
        % Get Software values
        obj = obj.readPVTable;
        exp_pv = obj.OutPVTable.PV;
        
        % Get common points
        digu_time = data.performancedata.date;
        exp_time = obj.OutPVTable.Time;
        [~, q, w] = intersect(digu_time, exp_time);
        digu_pv = digu_pv(q);
        exp_pv = exp_pv(w);
        
        % Find similarity
        p = polyfit(digu_pv*100, exp_pv, 1);
        slope = p(1);
        
        % Plot comparison
        figure;
        plot(digu_pv*100, exp_pv, 'b*');
        annotation('textbox',[.2 .5 .3 .3],...
                    'String',['Slope = ', num2str(slope)],...
                    'FitBoxToText','on');
    end
    
    function obj = zeroWindCorrection(obj)
    % zeroWind
    
        % Read table
        rawTbl = readtable(obj.WindFile);
        
        % Change values
        rawTbl.Var2 = zeros(height(rawTbl), 1);
        
        % Write to file
        writetable(rawTbl, obj.WindFile, 'WriteVariableNames', false);
    end
...
%         % Raw file
%         [newFilePath_ch, newFile_ch, newFileExt_ch] = fileparts(obj.RawFile);
%         newFile_ch = strcat(newFile_ch, '_mps');
%         copyfile(obj.RawFile, fullfile(newFilePath_ch, [newFile_ch, newFileExt_ch]));
%         obj = obj.readInputTable;
%         input_tbl = obj.InputTable;
...
%         writetable(input_tbl, obj.RawFile, 'WriteVariableNames', false);
%         
%         % Read files
%         sp1_tbl = readtable('L:\Project\MWB-Fuel efficiency\Hull and propeller performance\ISO 19030 Measurements of changes in hull and propeller performance\Validation\2. Calculation of Performance Values\500000 0_mps.csv');
%         sp2_tbl = readtable('L:\Project\MWB-Fuel efficiency\Hull and propeller performance\ISO 19030 Measurements of changes in hull and propeller performance\Validation\2. Calculation of Performance Values\1000000 -5_mps.csv');
%         
%         % Change values
...
%         
%         % Write files
%         writetable(sp1_tbl, 'L:\Project\MWB-Fuel efficiency\Hull and propeller performance\ISO 19030 Measurements of changes in hull and propeller performance\Validation\2. Calculation of Performance Values\500000 0.csv', 'WriteVariableNames', false);
%         writetable(sp2_tbl, 'L:\Project\MWB-Fuel efficiency\Hull and propeller performance\ISO 19030 Measurements of changes in hull and propeller performance\Validation\2. Calculation of Performance Values\1000000 -5.csv', 'WriteVariableNames', false);
    end
    
    function obj = findModelUsedInSoftware(obj)
    % 
    ...
    % Get expected speed
    sl = obj.OutPVTable.PV;
    vm = obj.OutPVTable.SpeedWater;
    ve = (1e2 .* vm) ./ (sl + 100);
    
    % Get corrected power
    cp = obj.OutPVTable.CorrectedPower;
    
    % Filter by values of one speed power curve only
%     dbTime = obj.DBISOTable.datetime_utc;
%     outTime = obj.OutPVTable.Time;
%     [~, w] = intersect(outTime, dbTime);
%     firstSP_l = obj.DBISOTable.nearestdisplacement == 5e5;
%     firstSP_l = firstSP_l(w);
%     ve = ve(firstSP_l);
%     cp = cp(firstSP_l);
    
    % Fit to a range of polynomial models
    nCurves = 4;
    nPoints = 1e3;
    x = repmat(linspace(min(ve), max(ve), nPoints)', 1, nCurves);
    y = nan(size(x));
    fit_c = cell(1, nCurves);
    
    for pi = 1:nCurves
    
        currFit = polyfit(ve, cp, pi);
        fit_c(pi) = {currFit};
        
        y(:, pi) = polyval(currFit, x(:, 1));
    end
    
    % Plot models against data
    figure;
    plot(x, y);
    hold on
    plot(ve, cp, 'b*');
