...
properties(Constant)
    ...
    SPFile cell = {fullfile(fileparts(mfilename('fullpath')), '500000 0.csv'), fullfile(fileparts(mfilename('fullpath')), '1000000 -5.csv')};
...
%     blockSize(blockSize == 0) = [];
    ...
    function obj = compareOutputsDitte(obj)
    % compareOutputsDitte
    
    % Get DB results
    obj = obj.readDBISOTable;
    db_tbl = obj.DBISOTable;
    db_v = db_tbl.corrected_power;
    
    % Get DIGU results
    digu_tbl = readtable(obj.DiguFile);
    digu_v = digu_tbl.cor_power;
    
    % Plot
    plot(db_tbl.datetime_utc, db_v, 'b*');
    hold on;
    plot(digu_tbl.date, digu_v, 'ro');
    hold off;
    ...
    % runISOOnInput 
        ...
        % Modify data
        raw_tbl = readtable(obj.RawFile);
        raw_tbl.Var5 = randi([0, 10], size(raw_tbl, 1), 1);
%         raw_tbl.Var5 = ones(size(raw_tbl, 1), 1);
        raw_tbl.Var6 = randi([0, 18], size(raw_tbl, 1), 1) * 10;
        raw_tbl.Var8 = randi([0, 10], size(raw_tbl, 1), 1); % raw_tbl.Var5;
        raw_tbl.Var9 = randi([0, 18], size(raw_tbl, 1), 1) * 10; % raw_tbl.Var6;
        raw_tbl.Var12 = randi([0, 10], size(raw_tbl, 1), 1); % repmat(5, size(raw_tbl, 1), 1); % abs(randn(size(raw_tbl, 1), 1)) + 10; % 
        raw_tbl.Var13 = randi([0, 10], size(raw_tbl, 1), 1); % repmat(9, size(raw_tbl, 1), 1); % abs(randn(size(raw_tbl, 1), 1)) + 10; % 
        raw_tbl.Var7 = repmat(0, size(raw_tbl, 1), 1);   % repmat(79.8274, size(raw_tbl, 1), 1);
        writetable(raw_tbl, obj.RawFile, 'WriteVariableNames', false);
        ...
        % Delete test vessel data already in database
        vessel.execute(['DELETE FROM PerformanceData WHERE IMO_Vessel_Number = ',...
            num2str(vessel.IMO_Vessel_Number), ';']);
        ...
        % Convert units
        obj = convertSoftwareInputsToKnots(obj);
    end
    
    function obj = convertSoftwareInputsToKnots(obj)
    % convertSoftwareInputsToKnots
...
%        
%        currFile = sp_c{fi};
%        curr_tbl = readtable(currFile);
%        curr_tbl{:, 1} = curr_tbl{:, 1}*mPs2Knots;
%        writetable(curr_tbl, currFile, 'WriteVariableNames', false);
%     end
...
methods(Test)
    ...
    function testTrueWindSpeed(testcase)
...
%     text(act_date, act_sl, diag(commonFromDB), 'HorizontalAlignment', 'Right')
    ...
    outVars = {'Time', 'CorrectedPower'};
    dbVars = lower({'DateTime_UTC', 'Corrected_Power'});
    ...
    testcase.verifyEqual(act_Corr, exp_Corr, 'RelTol', 5e-6, msg_Corr);
