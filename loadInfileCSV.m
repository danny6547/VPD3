function loadInfileCSV(csvfile, database, table)
% loadInfileCSV Load raw file from DNVGL into table in database
% loadInfileCSV(csvfile, database, table) will call SQL function LOAD
...
% Inputs
validateattributes(csvfile, {'cell', 'char'}, {'vector'}, 'loadInfileCSV',...
    'csvfile', 1);
if iscell(csvfile)
    cellfun(@(x) validateattributes(x, {'char'}, {'vector'}, ...
        'loadInfileCSV', 'csvfile', 1), csvfile);
end
