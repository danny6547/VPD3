/* Get performance data for IMO, optionally returning only requested dry-docking interval */
...
proc_label:BEGIN
    ...
    SET numDD = (SELECT COUNT(*) FROM `test16`.DryDockDates WHERE IMO_Vessel_Number = imo);
	...
	SET intervalStartDate = (SELECT EndDate FROM `test16`.DryDockDates WHERE IMO_Vessel_Number = imo LIMIT ddiBefore, 1);
	SET intervalEndDate = (SELECT StartDate FROM `test16`.DryDockDates WHERE IMO_Vessel_Number = imo LIMIT ddiAfter, 1);
    ...
    SET dateMissing = (SELECT COUNT(intervalEndDate) FROM `test16`.DryDockDates WHERE IMO_Vessel_Number = imo) = 0;
