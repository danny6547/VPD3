function [ guarStruct ] = guaranteeDurations(perData, varargin)
...
guarStruct = struct('StartMonth', [1, 13, 13, 13, 13], ...
                    ...
                    'Difference', [], ...
                    'RelativeDifference', []);
guarStruct = repmat(guarStruct, size(perData));
...
for ii = 1:numel(perData)
   ...
   % Indices to data
   dat = unique(datenum(currData.DateTime_UTC, 'dd-mm-yyyy'));
   per = currData.Performance_Index * 100;
   [dat, dati] = sort(dat);
   per = per(dati);
